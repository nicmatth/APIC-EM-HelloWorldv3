...
if res is not None:
    ...
    for node in path:
        ...
        if start_interface is None:
            ...
            print(start_host['hostIp'], "->", end=' ')
        else:
            print("->", start_interface['portName'], "--", end=' ')
        if end_interface is None:
            end_host = get_byid("host", node['target'])
            print(end_host['hostIp'])
        else:
            print(end_interface['portName'], "->", end=' ')
            print(end_device['hostname'], end=' ')

    print("\n******\nThis looks like a 'traceroute' but you can", end=' ')
